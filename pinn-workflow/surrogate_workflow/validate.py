...
def plot_trend(param_name, model, dataset, device, path):
    ...
    sweep = np.linspace(ranges[param_name][0], ranges[param_name][1], config.TREND_SWEEP_POINTS)
    ...
    x_norm, _, _ = data_utils.normalize_inputs(
        np.column_stack([sweep if name == param_name else np.full_like(sweep, mu_mid[i])
...
def optimization_safety_check(model, dataset, device):
    ...
    candidates = data_utils.sample_designs(config.OPT_CANDIDATES, ranges, config.SEED + 13)
    ...
    mu_mid = np.array([(ranges[name][0] + ranges[name][1]) * 0.5 for name in params])
