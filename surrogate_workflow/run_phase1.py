...
def load_or_generate_dataset(regenerate):
...
def build_loaders(dataset, seed):
    n_samples = dataset["x_norm"].shape[0]
    train_idx, val_idx, test_idx = data_utils.split_indices(
        n_samples, config.TRAIN_FRACTION, config.VAL_FRACTION, seed
    )
...
def main():
    parser = argparse.ArgumentParser(description="Phase 1 surrogate pipeline")
    ...
    model, history = surrogate.train_model(model, train_loader, val_loader, config, device)
